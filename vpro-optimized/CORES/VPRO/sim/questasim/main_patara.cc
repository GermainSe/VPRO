...
SC_MODULE(main)
{
	...
	if (dfile.is_open()) {
	  ...
	  cout << "[TB] Initializing DMEM with ZEROS and random DATA" << endl;
	  dmem_addr_int = 0;
	  while (dmem_addr_int < 64) {
	    ...
	    // Use random value
	    DMEM[dmem_addr_int++] = random_value;
	  }
        }
	...
	sc_spawn([&]{ 
	  ...
	  // +++++++++++++++++++++++++++++++++++++++++++++++++++++++
	  // Dump results from memory 0x000 and 0x001
	  // +++++++++++++++++++++++++++++++++++++++++++++++++++++++
	  std::ofstream outFile_mem("app/dump.log");
	  if (!outFile_mem.is_open()) {
            std::cerr << "Error: Unable to open file dump.log for writing." << std::endl;
            return;
	  }
	  // Set output stream formatting for hexadecimal address
	  outFile_mem << std::hex << std::setfill('0');
	  ...
	  for (int i = 0; i < 2; ++i) {
	    ...
	    outFile_mem << "(0x" << std::setw(4) << std::setfill('0') << std::hex << i << ")\t";
	    outFile_mem << "0x" << upperStream.str() << lowerStream.str() << std::endl;
	  }
	  // Close the file
	  outFile_mem.close();
