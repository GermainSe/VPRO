...
import weights_loader_yolo_lite_fpf
...
# def tf_inference(input_r, input_g, input_b, tf_wghts_path, out_node):
    ...
    _ = weights_loader_yolo_lite_fpf.load_fpf(sess, tf_wghts_path, ckpt_folder_path, saver)
    ...
    predictions = sess.run(out_node, feed_dict={net_yolo_lite_fpf.x: preprocessed_image})

    return predictions


# tf_verification
# net_out: net_yolo_lite_fpf.layer_out
...
def main(_):   # _: weights_loader
    ...
    args = net_yolo_lite_fpf.input_args()
    ...
    num_conv = net_yolo_lite_fpf.n_kernel  # number of convolution
    num_bias = net_yolo_lite_fpf.n_bias  # number of bias
    ...
    layer_out = net_yolo_lite_fpf.layer_out
    print("Gonna Perform inference!!\n")
    tf_reference_fpf(input_width, input_height, input_img_path, wghts_path, layer_out)
    ''''
    conv2d_out = net_yolo_lite_fpf.conv2d_out
    biasadd_out = net_yolo_lite_fpf.biasadd_out
    relu_out = net_yolo_lite_fpf.relu_out
    pool_out = net_yolo_lite_fpf.pool_out
