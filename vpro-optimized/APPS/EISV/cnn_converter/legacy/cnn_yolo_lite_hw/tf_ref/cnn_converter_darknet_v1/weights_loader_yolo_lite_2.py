...
def load(sess, weights_path, ckpt_folder_path, saver):
    ...
    while layer < len(net_yolo_lite_2.network_structure):
        if len(net_yolo_lite_2.network_structure[layer]) > 2:
            biases,kernel_weights,offset = load_conv_layer('conv'+str(layer),loaded_weights,
                       [net_yolo_lite_2.network_structure[layer][0],net_yolo_lite_2.network_structure[layer][1],
                        net_yolo_lite_2.network_structure[layer][2],net_yolo_lite_2.network_structure[layer][3]],
                       offset)
            sess.run(tf.compat.v1.assign(net_yolo_lite_2.bias_weights[count], biases))
            sess.run(tf.compat.v1.assign(net_yolo_lite_2.conv_weights[count], kernel_weights))
            print("Bias ",count, ":    ", np.min(biases), " - ", np.max(biases))
            print("Weights ",count, ": ", np.min(kernel_weights), " - ", np.max(kernel_weights))
            count += 1
        layer += 1
