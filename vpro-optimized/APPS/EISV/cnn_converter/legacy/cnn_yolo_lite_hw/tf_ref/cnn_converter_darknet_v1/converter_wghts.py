...
import weights_loader_yolo_lite
import binary_detection
...
def tf_inference(input_width, input_height, input_img_path, tf_wghts_path, out_node):
    ...
    _ = weights_loader_yolo_lite.load(sess, tf_wghts_path, ckpt_folder_path = './ckpt_yolo_lite/', saver=saver)

    # get input image
    preprocessed_image = tf_preprocessing(input_img_path, input_height, input_width)

    # pass input into the network defined in net_yolo_lite.py
    predictions = sess.run(out_node, feed_dict={net_yolo_lite.x: preprocessed_image})
...
def weights2h_max(input_width, input_height, input_img_path, wghts_path):
    ...
    bit_width_conv_input = range_log(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.conv_input)
    print(bit_width_conv_input)

    print("Fmap maximal bit widths (coeff of conv)")
    bit_width_weights = range_log(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.conv_weights)
    print(bit_width_weights)

    print("Fmap maximal bit widths (input of bias)")
    bit_width_bias_input = range_log(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.bias_input)
    print(bit_width_bias_input)

    print("maximal bit width of bias's")
    bit_width_bias = range_log(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.bias_weights)
    print(bit_width_bias)

    print("Fmap maximal bit widths (output of bias)")
    bit_width_bias_output = range_log(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.bias_output)
    print(bit_width_bias_output)

    print("Fmap maximal bit widths (after relu+pool)")
    bit_width_final = range_log(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.pool_output)
    ...
    with open(wght_tmp, 'w', newline='\n') as f:
        ...
        for i in range(len(net_yolo_lite.conv_weights)):
            print("\n>> Analyse (Conv) Layer ", i)

            ##########
            ##### CONV
            ##########
            conv_params = tf_inference(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.conv_weights[i])
            ...
            ##### BIAS
            ##########
            bias_params = tf_inference(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.bias_weights[i])
        ...
        print("int16_t conv_result_shift_right[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_result_shift, ";", file=f)
        # print("int16_t conv_input_int_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_input_int_precision, ";", file=f)
        # print("int16_t conv_input_frac_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_input_frac_precision, ";", file=f)
        # print("int16_t conv_int_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_int_precision, ";", file=f)
        # print("int16_t conv_frac_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_frac_precision, ";", file=f)
        # print("int16_t conv_result_int_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_result_int_precision, ";", file=f)
        # print("int16_t conv_result_frac_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", conv_result_frac_precision, ";", file=f)
        print("int16_t bias_store_shift_right[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_store_shift_right, ";", file=f)
        print("int16_t bias_load_shift_right[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_load_shift_right, ";", file=f)
        # print("int16_t bias_input_int_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_input_int_precision, ";", file=f)
        # print("int16_t bias_input_frac_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_input_frac_precision, ";", file=f)
        # print("int16_t bias_int_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_int_precision, ";", file=f)
        # print("int16_t bias_frac_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_frac_precision, ";", file=f)
        # print("int16_t bias_result_int_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_result_int_precision, ";", file=f)
        # print("int16_t bias_result_frac_precision[",len(net_yolo_lite.conv_weights),"] = \narray(", bias_result_frac_precision, ";", file=f)
...
def main(_):
    ...
    # ------------------------------------------------------------ #
    # get final results
    output = tf_inference(input_width, input_height, input_img_path, wghts_path, net_yolo_lite.pool_output)
    layer_number = len(output) - 1
    output = np.array(output[layer_number])
    output = np.reshape(output,(7, 7, 125))

    # check results in image
    img = binary_detection.postprocessing(output, input_img_path, 0.5, 0.3, input_height, input_width)
    ...
    output_reload = binary_detection.binary_inference(tf_out_bin_path, 7, 7, channel_number)
    ...
    img = binary_detection.postprocessing(output_reload, input_img_path, 0.5, 0.3, input_height, input_width)
