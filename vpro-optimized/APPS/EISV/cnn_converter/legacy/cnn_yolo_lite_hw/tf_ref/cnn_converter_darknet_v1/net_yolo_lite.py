...
while layer < len(network_structure):
    ...
    conv_input_shape.append(tf.shape(conv_input[-1])) # e.g. for layer 0: [1, 224, 224, 3]
    # print("perform conv: ")
    ...
    conv_output.append(tf.nn.conv2d(conv_input[-1], conv_weights[-1], strides=[1, 1, 1, 1], padding='SAME'))
    conv_output_shape.append(tf.shape(conv_output[-1]))
    ...
    if len(network_structure) > layer+1:
        ...
        pool_output.append(relu_output[-1])
    pool_output_shape.append(tf.shape(pool_output[-1]))
    # print(pool_output[-1])
    layer += 1
    # print(n_params)
