...
def load_fpf(sess, weights_path, ckpt_folder_path, saver):
    ...
    sess.run(tf.assign(net_yolo_lite_fpf.b[0], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[0], kernel_weights))

    # Conv1 , 3x3, 16->32
    biases, kernel_weights, offset = load_conv_layer('conv1', loaded_weights, [3, 3, 16, 32], offset)
    sess.run(tf.assign(net_yolo_lite_fpf.b[1], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[1], kernel_weights))

    # Conv2 , 3x3, 32->64
    biases, kernel_weights, offset = load_conv_layer('conv2', loaded_weights, [3, 3, 32, 64], offset)
    sess.run(tf.assign(net_yolo_lite_fpf.b[2], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[2], kernel_weights))

    # Conv3 , 3x3, 64->128
    biases, kernel_weights, offset = load_conv_layer('conv3', loaded_weights, [3, 3, 64, 128], offset)
    sess.run(tf.assign(net_yolo_lite_fpf.b[3], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[3], kernel_weights))

    # Conv4 , 3x3, 128->128
    biases, kernel_weights, offset = load_conv_layer('conv4', loaded_weights, [3, 3, 128, 128], offset)
    sess.run(tf.assign(net_yolo_lite_fpf.b[4], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[4], kernel_weights))

    # Conv5 , 3x3, 128->256
    biases, kernel_weights, offset = load_conv_layer('conv5', loaded_weights, [3, 3, 128, 256], offset)
    sess.run(tf.assign(net_yolo_lite_fpf.b[5], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[5], kernel_weights))

    # Conv6 , 1x1, 256->125
    biases, kernel_weights, offset = load_conv_layer('conv6', loaded_weights, [1, 1, 256, 125], offset)
    sess.run(tf.assign(net_yolo_lite_fpf.b[6], biases))
    sess.run(tf.assign(net_yolo_lite_fpf.w[6], kernel_weights))
