...
inline void _conv(const BIF::LAYER &layer, const uint16_t buffer) {
    if (layer.parallel_outchannels_per_lane > 1){   // implies kernel_size == 1, pool == 1, stride == 1
      // for loop to cut limit of segm block size to 63 (gamma to get to next n)
      // load without gamma / reuse input block
      assert(layer.seg_out_w >= 5);  // TODO: vector length compensate
...
#endif
      uint32_t offset = 0;
      for (int n = 0; n < layer.parallel_outchannels_per_lane; ++n) {
#if RV_VPRO_EXT == 1 and not defined(SIMULATION)
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::src1_all, VPRO_PARAMETER_INDIZES::src2_all, NoTrigger>(
                    SRC1_ADDR(RF_KERNEL_BASE + n, 0, 0, 0), // TODO: if kernel size != 1 -> + n * kernel
                    complex_ADDR_3D(SRC_SEL_LS, RF_BIAS_BASE + n, 0, 0, 0));
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::nowhere, VPRO_PARAMETER_INDIZES::dst_all, Trigger>(0, DST_ADDR(offset, 1, layer.seg_out_w, 0));
#else
            VPRO::DIM3::PROCESSING::mach_init_addr(L0_1,
                                                   DST_ADDR(offset, 1, layer.seg_out_w, 0),
                                                   SRC1_ADDR(RF_KERNEL_BASE + n, 0, 0, 0),    // 1015
                                                    SRC2_LS_3D,
                                                   layer.seg_out_w - 1,
                                                   layer.seg_out_h - 1,
                                                   0,
                                                   RF_BIAS_BASE + n, 0, 0, 0,  // 1014
                                                   false, true);
#endif
            offset += layer.seg_out_w * layer.seg_out_h;
      }
    } else if (kernel_x == 1) {
        assert((unsigned int) layer.seg_out_w - 1 <= MAX_X_END);
        assert((unsigned int) layer.seg_out_h - 1 <= MAX_Y_END);
        ...
        VPRO::DIM3::PROCESSING::mach_init_addr(L0_1,
                                               ...
                                               SRC1_ADDR(RF_KERNEL_BASE, 0, 0, 0),    // 1015
                                               SRC2_LS_3D,
        ...
        for (size_t y_out = 0; y_out < out_h; ++y_out) {
            ...
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::dst_all, VPRO_PARAMETER_INDIZES::src2_all, Trigger>(
            ...
            VPRO::DIM3::PROCESSING::mach_init_addr(L0_1,  // shift right by 3 (?)
                                                   ...
                                                   SRC1_ADDR(RF_KERNEL_BASE, 1, kernel_x, 0),    // 1015
                                                   SRC2_LS_3D,
                                                   kernel_x-1, kernel_y-1, layer.seg_out_w - 1, // x_end, y_end, z_end
                                                   RF_BIAS_BASE, 0, 0, 0,  // 1014
                                                   false, true);
...
inline void _conv_add(const BIF::LAYER &layer, const uint16_t buffer) {
    if (layer.parallel_outchannels_per_lane > 1) { // implies kernel_size == 1, pool == 1, stride == 1
        ...
        // for loop to cut limit of segm block size to 63 (gamma to get to next
        // n) load without gamma / reuse input block
        assert(layer.seg_out_w >= 5); // TODO: vector length compensate
        assert((unsigned int)layer.seg_out_w - 1 <= MAX_X_END);
        assert((unsigned int)layer.seg_out_h - 1 <= MAX_Y_END);
        assert((unsigned int)layer.seg_in_w <= MAX_BETA);
        assert((unsigned int)layer.seg_out_w <= MAX_BETA);
#if RV_VPRO_EXT == 1 and not defined(SIMULATION)
        c_vpro_lw<5, VPRO_PARAMETER_INDIZES::nowhere, VPRO_PARAMETER_INDIZES::src2_imm, Trigger>(
            0, SRC2_IMM_3D(buffer));
#else
        VPRO::DIM3::LOADSTORE::loads(buffer, 0, 1, layer.seg_in_w, 0,
                                     layer.seg_out_w - 1, layer.seg_out_h - 1,
                                     layer.parallel_outchannels_per_lane - 1);
        ...
        for (int n = 0; n < layer.parallel_outchannels_per_lane; ++n) {
            ...
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::src1_all, VPRO_PARAMETER_INDIZES::src2_all, NoTrigger>(
                SRC1_ADDR(RF_KERNEL_BASE + n, 0, 0, 0), // TODO: if kernel size != 1 -> + n * kernel
                complex_ADDR_3D(SRC_SEL_LS, offset, 1, layer.seg_out_w, 0));
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::nowhere, VPRO_PARAMETER_INDIZES::dst_all, Trigger>(
                0, DST_ADDR(offset, 1, layer.seg_out_w, 0));
#else
