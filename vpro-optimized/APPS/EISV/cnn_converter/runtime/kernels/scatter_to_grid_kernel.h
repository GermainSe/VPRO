...
#define PP_MAX_POINTS 2000
#define PP_GRID_X 120
#define PP_GRID_Y 120
#define PP_OUT_CH 32
#define GRID_SIZE PP_GRID_Y * PP_GRID_X
#define GRID_SIZE_TOTAL GRID_SIZE * PP_OUT_CH * sizeof(int16_t)
...
inline void scatter_to_grid_riscv(const LAYER &layer, const COMMAND_SEGMENT *commands, const uint32_t cmd_size) {
    ...
    static int16_t __attribute__((section(".nobss"))) x[PP_MAX_POINTS];
    static int16_t __attribute__((section(".nobss"))) y[PP_MAX_POINTS];
    static uint32_t __attribute__((section(".nobss"))) indices[PP_MAX_POINTS];

    // arrays for grid indices and features located in riscv data-cache
    static int16_t __attribute__((section(".nobss"))) features[PP_MAX_POINTS];
...
inline void scatter_to_grid_vpro_dma(const LAYER &layer, const COMMAND_SEGMENT *commands, const uint32_t cmd_size) {
    ...
    static int16_t __attribute__((section(".nobss"))) y[PP_MAX_POINTS];
    static uint32_t __attribute__((section(".nobss"))) indices[PP_MAX_POINTS];
    static int16_t __attribute__((section(".nobss"))) features[PP_OUT_CH][PP_MAX_POINTS];

    // initializing the grid with zeros before max-pooling only works, if features are positive,
    // e.g. a ReLU preceeds the scattering. Otherwise grid would need to be initialized with -2**15
    //static int16_t __attribute__((section(".data")))
    int16_t grid[PP_OUT_CH][GRID_SIZE] = {0};
    aux_flush_dcache();
    ...
    while (transferred_bytes < GRID_SIZE_TOTAL) {
        for (uint32_t cl=0x1; cl <= last_valid_cluster && transferred_bytes < GRID_SIZE_TOTAL; cl <<= 1, transferred_bytes+=memcopy_size_bytes) {
