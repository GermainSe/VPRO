...
//    // VPRO MACH (conv loop)
            ...
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::chain_blocking_update_flag, VPRO_PARAMETER_INDIZES::x_y_z_end, NoTrigger>(0b001,
                                                                                                         (uint32_t(layer.seg_out_w - 1) << (10 + 6)) |
                                                                                                             (uint32_t(layer.seg_out_h - 1) << 10) | 0);
...
//    // VPRO MACH (conv loop)
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::id, VPRO_PARAMETER_INDIZES::func, NoTrigger>(L0_1, FUNC_MACH);
            c_vpro_lw<6, VPRO_PARAMETER_INDIZES::src1_all, VPRO_PARAMETER_INDIZES::x_y_z_end, NoTrigger>(SRC1_ADDR(RF_KERNEL_BASE, 1, kernel_x, 0),
                                                                                                         uint32_t(kernel_x - 1) << (10 + 6) | uint32_t(kernel_y - 1) << 10 |
                                                                                                         uint32_t(layer.seg_out_w - 1));
