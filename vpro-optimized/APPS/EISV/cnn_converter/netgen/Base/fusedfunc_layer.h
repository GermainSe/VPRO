...
namespace CNN_LAYER {
    ...
    public:
        ...
        virtual BIF::COMMAND_SEGMENT shiftStoreVPRO(BIF::COMMAND_VPRO &mem_layout, BUFFER &store_buffer) {
            ...
            if (cmd.vpro.zend > 0){
              if (cmd.vpro.rf_ch_stride > MAX_GAMMA || cmd.vpro.lm_lane_stride > MAX_GAMMA){
                  // not for upsample!
                  assert(upsampling_scale == 1);
                  // merge to large gamma to x/y iteration
                  // Modify both parameters (LS + processing instruction) -> check lane_stride values
                  assert(cmd.vpro.rf_ch_stride == cmd.vpro.lm_ch_stride);
                  // no gap between z iterations -> check if merge is possible
                  assert((cmd.vpro.yend + 1) * (cmd.vpro.xend + 1) == cmd.vpro.rf_ch_stride);

                  // single block using iterator z
                  cmd.vpro.zend = (cmd.vpro.yend + 1) * (cmd.vpro.xend + 1) * (cmd.vpro.zend + 1) - 1;
                  cmd.vpro.yend = 0;
                  cmd.vpro.xend = 0;
                  cmd.vpro.rf_ch_stride = 1;    // becomes gamma
                  cmd.vpro.lm_ch_stride = 1;
              }
            }
