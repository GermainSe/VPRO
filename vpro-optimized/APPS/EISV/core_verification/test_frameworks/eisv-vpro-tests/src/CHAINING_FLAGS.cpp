...
int main(int argc, char *argv[]) {
    ...
    // execute test
//                (int32_t(int16_t(opa[i] & 0xff)) - int32_t(0x7f)) >= 0) => 0xff
    // DMA load 1 to 0 [+64]
    ...
    // truncate input vector to the lowest 8 bits // = LOADB
    //LD (from: 0, size: 64, to: 0)
    __vpro(L1, NONBLOCKING, NO_CHAIN, FUNC_AND, NO_FLAG_UPDATE, DST_ADDR(0, 1, 8),
                                    SRC1_LS_2D, SRC2_IMM_2D(0xff), 7, 7);
    __vpro(LS, NONBLOCKING, IS_CHAIN, FUNC_LOADS, NO_FLAG_UPDATE, DST_ADDR(0, 1, 8),
                                    SRC1_ADDR(0, 1, 8), SRC2_IMM_2D(0), 7, 7);

    // subtract (REVERSED OPERANDS!) threshold (0x7F) from input vector, set flags and store to dummy vector (1023)
    // move MAX (0xFF) to result vector (128..191) if plus condition (in dummy vector (1023))
    __vpro(L0, NONBLOCKING, NO_CHAIN, FUNC_MV_PL, NO_FLAG_UPDATE, DST_ADDR(128, 1, 8),
                                    SRC1_CHAINING_RIGHT_2D, SRC2_IMM_2D(0xFF), 7, 7);
    __vpro(L1, NONBLOCKING, IS_CHAIN, FUNC_SUB, FLAG_UPDATE, DST_ADDR(1023, 0, 0),
                                    SRC2_IMM_2D(0x7F), SRC1_ADDR(0, 1, 8), 7, 7);

    //ST (from: 1, size: 64, to: 128)
