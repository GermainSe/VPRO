...
int main(int argc, char *argv[]) {
    ...
    // execute test
    // Load 2D
    dma_e2l_2d(0b1, 0b1, uint64_t(intptr_t(&(test_array_1[0]))), 128, 4, 8, 4 + 1);       // left half
    dma_e2l_2d(0b1, 0b1, uint64_t(intptr_t(&(test_array_1[4]))), 128 + 32, 4, 8, 4 + 1);    // left half //
    dma_wait_to_finish(0xffffffff);
    // Writeback 1D
    dma_l2e_1d(0b1, 0b1, uint64_t(intptr_t(result_array)), 128, NUM_TEST_ENTRIES); // different order
    dma_wait_to_finish(0xffffffff);
