...
int main(int argc, char *argv[]) {
    ...
    // execute test
    // Set RF [00-31] to 0x1000
    __vpro(L0, NONBLOCKING, NO_CHAIN, FUNC_ADD, NO_FLAG_UPDATE, DST_ADDR(0, 1, 8),
                                    SRC2_IMM_2D(0), SRC2_IMM_2D(0x1000), 7, 3);
    // Set RF [32-63] to 0x2000
    __vpro(L0, NONBLOCKING, NO_CHAIN, FUNC_ADD, NO_FLAG_UPDATE, DST_ADDR(32, 1, 8),
                                    SRC2_IMM_2D(0), SRC2_IMM_2D(0x2000), 7, 3);
...
//                                    SRC1_CHAINING_2D(0), SRC2_IMM_2D(0), 7, 7);
    ...
    dma_l2e_2d(0b1, 0b1, uint64_t(intptr_t(result_array)), 128 + 16, 4, 8, 4 + 1);  // left part in mm
    dma_l2e_2d(0b1, 0b1, uint64_t(intptr_t(&(result_array[4]))), 128 + 16, 4, 8, 4 + 1);  // right part in mm // same content: 16 * 0x1000, 16 * 0x2000
    dma_wait_to_finish(0xffffffff);
