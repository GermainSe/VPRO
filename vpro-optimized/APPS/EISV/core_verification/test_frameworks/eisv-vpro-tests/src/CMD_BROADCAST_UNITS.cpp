...
int main(int argc, char *argv[]) {
    ...
    // execute test
    loadTestDataLM(0, const_cast<int16_t *>(test_array_1), NUM_TEST_ENTRIES / 2);
    loadTestDataLM(0, const_cast<int16_t *>(&(test_array_1[NUM_TEST_ENTRIES / 2])), NUM_TEST_ENTRIES / 2,
                   0, 1);
    dma_wait_to_finish(0xffffffff);
    ...
    __vpro(L0, NONBLOCKING, IS_CHAIN, FUNC_ADD, NO_FLAG_UPDATE, DST_ADDR(0, 1, 8),
                                    SRC1_ADDR(0, 1, 8), SRC2_IMM_2D(0), 7, 7);
...
//                                    SRC1_CHAINING_2D(0), SRC2_IMM_2D(0), 7, 7);
    ...
    dma_l2e_1d(0b1, 0b1, uint64_t(intptr_t(result_array)), 128, NUM_TEST_ENTRIES / 2);
    dma_l2e_1d(0b1, 0b10, uint64_t(intptr_t(&(result_array[NUM_TEST_ENTRIES / 2]))), 128,
                       NUM_TEST_ENTRIES / 2);
    dma_wait_to_finish(0xffffffff);
