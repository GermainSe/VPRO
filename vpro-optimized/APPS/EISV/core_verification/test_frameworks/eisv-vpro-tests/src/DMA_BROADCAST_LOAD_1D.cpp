...
int main(int argc, char *argv[]) {
    ...
    // execute test
    // Load 1D
    dma_e2l_1d(0b1, 0b11, uint64_t(intptr_t(test_array_1)), 128, NUM_TEST_ENTRIES);
    dma_wait_to_finish(0xffffffff);
    // Writeback 1D
    dma_l2e_1d(0b1, 0b01, uint64_t(intptr_t(&(result_array[0]))), 128, NUM_TEST_ENTRIES/2);
    dma_l2e_1d(0b1, 0b10, uint64_t(intptr_t(&(result_array[NUM_TEST_ENTRIES/2]))), 128 + NUM_TEST_ENTRIES/2, NUM_TEST_ENTRIES/2);
    dma_wait_to_finish(0xffffffff);
