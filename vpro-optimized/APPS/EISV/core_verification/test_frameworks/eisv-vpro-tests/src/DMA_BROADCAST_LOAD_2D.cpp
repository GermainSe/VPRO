...
int main(int argc, char *argv[]) {
    ...
    // execute test
    // Load 2D    
    dma_e2l_2d(0b1, 0b11, intptr_t(&(test_array_1[0])), 128,        4, 8, 4 + 1);
    dma_e2l_2d(0b1, 0b11, intptr_t(&(test_array_1[4])), 128 + 32,   4, 8, 4 + 1);
    
    dma_wait_to_finish(0xffffffff);
    // Writeback 2D
    dma_l2e_2d(0b1, 0b01, uint64_t(intptr_t(&(result_array[0]))), 128, 4, 8, 4 + 1);  // left part in mm
    dma_l2e_2d(0b1, 0b10, uint64_t(intptr_t(&(result_array[4]))), 128 + 32, 4, 8, 4 + 1);  // right part in mm // same content: 16 * 0x1000, 16 * 0x2000
    dma_wait_to_finish(0xffffffff);
