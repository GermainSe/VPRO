...
int main(int argc, char *argv[]) {
    ...
    //MACL_PRE (size: 64, to: 128, 1*0x1234)
    __vpro(L0, NONBLOCKING, NO_CHAIN, FUNC_MACL_PRE, NO_FLAG_UPDATE,
           DST_ADDR(128, 0, 0),
           SRC1_IMM_2D(1), SRC2_IMM_2D(0x1234), 0, 0);

    //LD (from: 0, size: 64, to: 0)
    __vpro(LS, NONBLOCKING, IS_CHAIN, FUNC_LOADS, FLAG_UPDATE, DST_ADDR(0, 1, 8),
                                    SRC1_ADDR(0, 1, 8), SRC2_IMM_2D(0), 7, 7);

    //MACH (from: 0 & 512, size: 64, to: 128)
    __vpro(L0, NONBLOCKING, NO_CHAIN, FUNC_MACH, NO_FLAG_UPDATE,
                                    DST_ADDR(128, 1, 8),
