    ...
    // CONV (+bias)
      if (kernel_x == 3 && kernel_y == 3){
        for (size_t y = 0; y < seg_out_h; ++y) {
          auto offset_in = y * seg_in_w;
          __vpro(LS, NONBLOCKING, IS_CHAIN, FUNC_LOADS, NO_FLAG_UPDATE,
                                          DST_ADDR(0, 0, 0, 0),
                                          SRC1_ADDR(offset_in, 1, seg_in_w, 1),
                                          SRC2_IMM_2D(buffer),
                                          2, 2, seg_out_w);

          auto offset_out = y * seg_out_w;
          __vpro(L0_1, NONBLOCKING, NO_CHAIN, FUNC_MACH_PRE, NO_FLAG_UPDATE,
                                          DST_ADDR(offset_out, 0, 0, 1),
                                          SRC1_LS_2D,
                                          SRC2_ADDR(RF_KERNEL_BASE, 1, 3, 0),
                                          2, 2, seg_out_w);
          //add bias
          __vpro(L0_1, NONBLOCKING, NO_CHAIN, FUNC_ADD, NO_FLAG_UPDATE,
                                          DST_ADDR(offset_out, 0, 0, 1),
                                          SRC1_ADDR(RF_BIAS_BASE, 0, 0, 0),
                                          SRC2_ADDR(offset_out, 0, 0, 1),
                                          0, 0, seg_out_w);
        }
    } else {
