...
#define SIMULATION_SIGNATURE_DUMP 0xffffffc4
#define SIMULATION_EXIT 0xffffffcc

#define RV_COMPLIANCE_HALT                                                    \
        la t5, SIMULATION_SIGNATURE_DUMP;                                     \
        la t0, begin_signature;                                               \
        la t1, end_signature;                                                 \
        _signature_return_loop:                                               \
        lw t2, 0(t0);                                                         \
        sw t2, 0(t5);                                                         \
        addi t0, t0, 4;                                                       \
        /* loop all signature addresses    */                                 \
        bne t0, t1, _signature_return_loop;                                   \
        la t5, SIMULATION_EXIT;                                               \
        /* exit code of application (sim) */                                  \
        sw gp, 0(t5);                                                         \
        RVTEST_PASS                                                           \
