...
bool CHECK_MUL_OPERAND_WIDTH_OVERFLOW_WARNING_WAS_PRINTED = false;
...
int ISS::sim_init(int (*main_fkt)(int, char**), int& argc, char* argv[]) {
    ...
    if (!windowThread) {
        ...
        printf("#  Main Memory (Non Blocking):    8-bit x %lu (%3.f %s)\n",
            ...
            str.c_str());
#endif
        printf("#  Local Memory:  16-bit x %6u\n", VPRO_CFG::LM_SIZE);
        printf("#  Register File: 24-bit x %6u\n", VPRO_CFG::RF_SIZE);
        printf("#\n");
        printf("# Clocks:\n");
        ...
        if (CREATE_CMD_ISSUE_FILE) {
            QFileInfo fi(CMD_ISSUE_FILE_NAME);

            if (!fi.absoluteDir().exists()) {
                QDir().mkdir(fi.absoluteDir().absolutePath());
                printf_info("[CMD_ISSUE_FILE] Directory for file created: %s\n",
                    fi.absoluteDir().absolutePath().toStdString().c_str());
            }
            CMD_ISSUE_FILE = fopen(CMD_ISSUE_FILE_NAME, "w");
            if (CMD_ISSUE_FILE == nullptr) {
                printf_error(
                    "CMD Issue file could not be opened! [FILE : %s]\n", CMD_ISSUE_FILE_NAME);
            }
        }
...
//    connect(this, SIGNAL(deleteLater()), &simulatorThread, SLOT(exitIssThread()));  // TODO: no such signal?! (with gui - debug | windowless)
    ...
    if (!windowless) {
        ...
        connect(w, SIGNAL(exit(int)), this, SLOT(exit(int)));           // TODO: no such signal?! (with gui - debug)
...
void ISS::sim_stop(bool silent, int exit_code) {
    ...
    if (CREATE_CMD_ISSUE_FILE) {
        fflush(CMD_ISSUE_FILE);
        fclose(CMD_ISSUE_FILE);
    }
    if (CREATE_CMD_PRE_GEN) {
        PRE_GEN_FILE->close();
    }
...
void ISS::dbgMemWrite(intptr_t mm_dst_addr, uint8_t* data_ptr, size_t size) {
    for (int i = 0; i < size; ++i) {
        // uses little endianess -> lsb -> 'low address, msb -> 'high address
        bus->dbgWrite(mm_dst_addr+i, data_ptr+i);
    }
}

void ISS::dbgMemRead(intptr_t mm_src_addr, uint8_t* data_ptr, size_t size) const {
    for (int i = 0; i < size; ++i) {
        bus->dbgRead(mm_src_addr+i, data_ptr+i);
    }
}
