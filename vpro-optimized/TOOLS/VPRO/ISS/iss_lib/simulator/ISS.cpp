...
void ISS::run_vpro_instruction(const std::shared_ptr<CommandVPRO>& command) {
    ...
    if (command->type == CommandVPRO::MV_ZE ||
        command->type == CommandVPRO::MV_NZ ||
        command->type == CommandVPRO::MV_MI ||
        command->type == CommandVPRO::MV_PL){
        if (command->is_chain){
            printf_error("Command of conditional move uses Chaining Output Fifo. "
                "This causes irregular vector lengths and therefore loops during execution!\n");
            command->print();
            sim_stop(false, 1);
            exit(1);
        }
    ...
    if (CREATE_CMD_ISSUE_FILE){
        command->print_cmd_issue_string(CMD_ISSUE_FILE);
    }
}
...
//            dcma_stat->reset();
        ...
        case (VPRO_CMD_REGISTER_3_2_ADDR): {  // Trigger, 2D
            ...
            io_vpro_cmd_register.dst_sel = SRC_SEL_ADDR;
...
std::shared_ptr<CommandVPRO> ISS::gen_io_vpro_command() const {
    ...
    command->src1.chain_neighbor = (io_vpro_cmd_register.src1_sel == SRC_SEL_NEIGHBOR) ||
                               (io_vpro_cmd_register.src1_sel == SRC_SEL_INDIRECT_NEIGHBOR);
    ...
    command->src2.chain_neighbor = (io_vpro_cmd_register.src2_sel == SRC_SEL_NEIGHBOR) ||
                               (io_vpro_cmd_register.src2_sel == SRC_SEL_INDIRECT_NEIGHBOR);
    ...
    command->dst.chain_neighbor = (io_vpro_cmd_register.dst_sel == SRC_SEL_NEIGHBOR) ||
                              (io_vpro_cmd_register.dst_sel == SRC_SEL_INDIRECT_NEIGHBOR);
