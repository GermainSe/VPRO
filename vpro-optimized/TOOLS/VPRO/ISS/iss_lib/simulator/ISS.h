...
class ISS : public QThread {
   ...
   public slots:
     ...
     */
    void sim_stop(bool silent = false, int exit_code = 0);
    ...
    void dbgMemWrite(intptr_t mm_dst_addr, uint8_t* data_ptr, size_t size = 1);

    void dbgMemRead(intptr_t mm_src_addr, uint8_t* data_ptr, size_t size = 1) const;
