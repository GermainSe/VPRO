...
bool CommandVPRO::isWriteLM() const {
...
bool CommandVPRO::isChainPartCMD() {
    return (is_chain || src1.sel == SRC_SEL_NEIGHBOR ||
            src1.sel == SRC_SEL_LS || src2.sel == SRC_SEL_NEIGHBOR ||
            src2.sel == SRC_SEL_LS);
}
...
QString CommandVPRO::get_string() {
    ...
    else if (src1.sel == SRC_SEL_NEIGHBOR)
        str += QString("SRC1(chain Neighbor") +
               QString("),    ");
    ...
    else if (src2.sel == SRC_SEL_NEIGHBOR)
        str += QString("SRC2(chain Neighbor") +
               QString("),    ");
...
uint32_t CommandVPRO::get_func_type() {
    switch (type) {
        ...
        case SHIFT_LL:
            printf_error("[VPRO Instruction ERROR] SHIFT_LL unsupported by hardware!\n");
...
uint32_t CommandVPRO::get_lane_dst_sel() const {
    return static_cast<uint32_t>(dst.sel);  // NOT USED!!
}

uint32_t CommandVPRO::get_src2_off() const {
...
void CommandVPRO::print_cmd_issue_string(FILE* out, const char *prefix) const {
    fprintf(out, "%s", prefix);

    if (isLS()){
        fprintf(out, "VPRO::DIM3::LOADSTORE::");
        switch (type) {
            case LOAD:              fprintf(out, "load");               break;
            case LOADB:             fprintf(out, "loadb");              break;
            case LOADS:             fprintf(out, "loads");              break;
            case LOADS_SHIFT_LEFT:  fprintf(out, "load_shift_left");    break;
            case LOADS_SHIFT_RIGHT: fprintf(out, "load_shift_right");   break;
            case LOAD_REVERSE:      fprintf(out, "load_reverse");       break;
            case LOADBS:            fprintf(out, "loadbs");             break;
            case STORE:             fprintf(out, "store");              break;
            case STORE_SHIFT_LEFT:  fprintf(out, "store_shift_left");   break;
            case STORE_SHIFT_RIGHT: fprintf(out, "store_shift_right");  break;
            case STORE_REVERSE:     fprintf(out, "store_reverse");      break;
            default:                fprintf(out, "nop");                break;
        }
        // TODO: dynamic_load, dynamic_loads, dynamic_loadb, dynamic_loadbs
        if (isWriteLM()){
            // store
            fprintf(out, "(%i, %i, %i, %i, %i, %i, %i, %i, %s",
                src2.getImm(),
                src1.offset, src1.alpha, src1.beta, src1.gamma,
                x_end, y_end, z_end,
                printLANE((LANE)(1<<dst.gamma)));   // src lane
        } else {
            // load
        ...
        fprintf(out, "VPRO::DIM3::PROCESSING::");
        switch (type) {
            ...
            case XOR:               fprintf(out, "xor_");               break;
            case XNOR:              fprintf(out, "xnor");               break;
            case AND:               fprintf(out, "and_");               break;
            case NAND:              fprintf(out, "nand");               break;
            case OR:                fprintf(out, "or_");                break;
            ...
            case MIN_VECTOR:        fprintf(out, "TODO_min_vector_value");   break; // TODO: min_vector_index
            case MAX_VECTOR:        fprintf(out, "TODO_max_vector_value");   break; // TODO: max_vector_index
            case BIT_REVERSAL:      fprintf(out, "TODO_bit_reversal");       break;
            case MV_ZE:             fprintf(out, "mv_zero");            break;
            case MV_NZ:             fprintf(out, "mv_non_zero");        break;
            case MV_MI:             fprintf(out, "mv_negative");        break;
            case MV_PL:             fprintf(out, "mv_non_negative");    break;
            case SHIFT_AR_NEG:      fprintf(out, "shift_ar_neg");       break;
            case SHIFT_AR_POS:      fprintf(out, "shift_ar_pos");       break;
        ...
        // TODO: bit_reversal -> src2 -> amount_of_bits
        //  max_vector_value / index / min -> no src2
        //  mach_init_imm -> src2 / LS const -> imm
        //  mach_init_addr -> src2 / LS const -> alpha/beta/gamma

        fprintf(out, "(%s, ", printLANE((LANE)id_mask));
        dst.print_cmd_issue_string(out);
        fprintf(out, ", ");
        src1.print_cmd_issue_string(out);
        fprintf(out, ", ");
        src2.print_cmd_issue_string(out);
        fprintf(out, ", ");
        fprintf(out, "%i, %i, %i", x_end, y_end, z_end);
        if (is_chain || blocking || flag_update){
            fprintf(out, ", %s, %s, %s",
                (is_chain)?"true":"false",
