...
bool VectorLane::is_indirect_addr(addr_field_t adr) const {
    return adr.sel == SRC_SEL_INDIRECT_NEIGHBOR ||
        ...
        switch (src.sel) {
            ...
            case SRC_SEL_INDIRECT_LS:
                if (isLSLane()) {  // for chaining between LS Lanes
                    printf_error("LS LANE Chaining not implemented!\n");
                }
                return getLSLane();
            default:
                return *this;
