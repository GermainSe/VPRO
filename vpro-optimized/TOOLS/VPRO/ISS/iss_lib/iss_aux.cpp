...
void dma_wait_to_finish(uint32_t cluster_mask) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_dma_sync(); // dma_wait_to_finish(0x%x);\n", cluster_mask);
    }
...
void dma_e2l_2d(uint32_t cluster_mask,
    ...
    if (CREATE_CMD_ISSUE_FILE){
        if (pad_flags[0] || pad_flags[1] || pad_flags[2] || pad_flags[3])
            fprintf(CMD_ISSUE_FILE, "dma_e2l_2d(0x%x, 0x%x, 0x%lx, 0x%x, %i, %i, %i, const_cast<bool[4]>({%i, %i, %i, %i}));\n",
                cluster_mask, unit_mask, ext_base, loc_base, x_size, y_size, x_stride,
                pad_flags[0], pad_flags[1], pad_flags[2], pad_flags[3]);
        else
            fprintf(CMD_ISSUE_FILE, "dma_e2l_2d(0x%x, 0x%x, 0x%lx, 0x%x, %i, %i, %i);\n",
                cluster_mask, unit_mask, ext_base, loc_base, x_size, y_size, x_stride);
    }
    uint32_t pad = 0;
    pad |= (pad_flags[0] << 0);
...
void dma_l2e_2d(uint32_t cluster_mask,
    ...
    uint32_t x_stride) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "dma_l2e_2d(0x%x, 0x%x, 0x%lx, 0x%x, %i, %i, %i);\n",
            cluster_mask, unit_mask, ext_base, loc_base, x_size, y_size, x_stride);
    }
    core_->io_write(IDMA_CLUSTER_MASK_ADDR, cluster_mask);
    core_->io_write(IDMA_UNIT_MASK_ADDR, unit_mask);
...
void dma_reset_access_counters() {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "dma_reset_access_counters();\n");
    }
...
void dcma_flush() {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "dcma_flush();\n");
    }
...
void dcma_reset() {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "dcma_reset();\n");
    }
    while (core_->io_read(DCMA_WAIT_BUSY_ADDR)) {}
...
void vpro_wait_busy(uint32_t cluster_mask, uint32_t unit_mask) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_lane_sync(); // vpro_wait_busy(0x%x, 0x%x);\n", cluster_mask, unit_mask);
    }
    core_->setWaitingToFinish(true);
    core_->io_write(VPRO_BUSY_MASK_CL_ADDR, cluster_mask);
...
void vpro_set_cluster_mask(uint32_t idmask) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_set_cluster_mask(%i);\n", idmask);
    }
...
void vpro_set_unit_mask(uint32_t idmask) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_set_unit_mask(%i);\n", idmask);
    }
...
void vpro_mac_h_bit_shift(uint32_t shift) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_mac_h_bit_shift(%i);\n", shift);
    }
...
void vpro_mul_h_bit_shift(uint32_t shift) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_mul_h_bit_shift(%i);\n", shift);
    }
...
void vpro_set_mac_init_source(VPRO::MAC_INIT_SOURCE source) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_set_mac_init_source(VPRO::MAC_INIT_SOURCE::%s);\n", print(source));
    }
...
void vpro_set_mac_reset_mode(VPRO::MAC_RESET_MODE mode) {
    if (CREATE_CMD_ISSUE_FILE){
        fprintf(CMD_ISSUE_FILE, "vpro_set_mac_reset_mode(VPRO::MAC_RESET_MODE::%s);\n", print(mode));
    }
