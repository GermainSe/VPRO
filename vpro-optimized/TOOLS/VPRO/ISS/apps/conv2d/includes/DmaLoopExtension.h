...
public:
    explicit DmaLoopExtension(std::vector<COMMAND> &cmd_list) : cmd_list(cmd_list) {}

    std::vector<COMMAND> generate(bool debug = false);
...
private:
     ...
     */
    std::list<COMMAND> extractLoopCommand(std::list<COMMAND> same_dim_dma_list, bool debug = false);
     ...
     */
    COMMAND generateLoopCommand(std::list<COMMAND> &loop_cmd_list, uint8_t cluster_shift, uint8_t unit_shift, int cluster_len, int unit_len,
     ...
     */
    bool verifyLoopCommand(std::list<COMMAND> &loop_cmd_list, COMMAND &loop_cmd, bool debug = false);
