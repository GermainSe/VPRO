...
std::vector<COMMAND> DmaLoopExtension::generate(bool debug) {
    ...
    std::list<COMMAND> new_list;

    auto begin = cmd_list.begin();
    auto end = begin;
    end++;

    auto cmp_func = [](const COMMAND &d1, const COMMAND &d2) -> bool {
        if (d2.type != DMA) return false;
        if (d1.type != DMA) return false;
...
//                    printf("  [DMA LOOP gen] Finish (new begin: %s)\n", end->to_char());
                new_list.splice(new_list.end(), extractLoopCommand(std::list<COMMAND>(begin, end), debug));
                begin = end;
...
//    auto sortfunction = [](const COMMAND &a, const COMMAND &b) -> bool{
...
//                if (debug)
//                    printf("  [DMA LOOP gen] Finish (new begin: %s)\n", end2->to_char());
//                newer_list.splice(newer_list.end(), extractLoopCommand(std::list<COMMAND>(begin2, end2), debug));
//                begin2 = end2;
//                is_dma_cmd_loop = false;
//                if (end2->dma.direction == loop){
//                    newer_list.push_back(*end2);
...
//        dump(cmd);
        if (!counting_block_cmds && (cmd.type == DMA || cmd.type == DMA_LOOP)){
            block = &cmd;
            if (cmd.type == DMA_LOOP){
                loop_encoded_dmas += reinterpret_cast<COMMAND_DMA_LOOP *>(&cmd)->dma_cmd_count;
                loop_count++;
            }
...
//                dump(*block);
    ...
    return std::vector<COMMAND>(newer_list.begin(), newer_list.end());
}

std::list<COMMAND> DmaLoopExtension::extractLoopCommand(std::list<COMMAND> same_dim_dma_list, bool debug) {
...
//            if (cnt > 3) break;
    ...
    auto cluster_mask_check = is_modified_base_mask(base.cluster, second.cluster);
    auto unit_mask_check = is_modified_base_mask(base.unit_mask, second.unit_mask);
    ...
    while (true) {
         ...
         */
        cluster_mask_check = is_modified_base_mask(base.cluster, second.cluster);
         ...
         */
        unit_mask_check = is_modified_base_mask(base.unit_mask, second.unit_mask);
        ...
        if (loop) {
            if (!is_looping) {    // first
                ...
                mm_incr = second.mm_addr - base.mm_addr;
                lm_incr = second.lm_addr - base.lm_addr;
                cluster_len = 0;
                unit_len = 0;
                inter_unit_len = 0;
              ...
              if (mm_incr != (int32_t)(second.mm_addr - previous.mm_addr)) {
                    if (debug)
                        printf("  \e[91m  [DMA LOOP Extract] Undo. No New Loop: mm != %i\e[m\n", (second.mm_addr - previous.mm_addr));
                    ...
                    if (loop_update_target == INTER_UNIT && cluster_mask_check.second == 0 && unit_mask_check.second == 0) {
                        if (lm_incr != (int32_t)(second.lm_addr - previous.lm_addr)) {
                            if (debug)
                                printf("  \e[91m  [DMA LOOP Extract] Undo. No New Loop: lm != %i (target: %i)\e[m\n", (second.lm_addr - previous.lm_addr), lm_incr);
                            loop = false; // TODO: loop will not be created -> fix to create one and continue
                        } else {
                            loop_update_target = INTER_UNIT;
                            inter_unit_len++;
...
//            printf("       %s\n", i.to_char());
    ...
    for (auto i : new_list) {
        if (i.type == DMA_LOOP){
            output_size += reinterpret_cast<COMMAND_DMA_LOOP *>(&i)->dma_cmd_count - 2;    // base + loop command
        }
    }
...
COMMAND DmaLoopExtension::generateLoopCommand(std::list<COMMAND> &loop_cmd_list, uint8_t cluster_shift, uint8_t unit_shift, int cluster_len,
                                                           int unit_len, int inter_unit_len, int mm_incr, int lm_incr, int count, bool debug) {
    COMMAND loop_cmd;

    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->type = DMA_LOOP;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->direction = COMMAND_DMA::DMA_DIRECTION::loop;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->mm_incr = mm_incr;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->lm_incr = lm_incr;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->cluster_loop_shift_incr = cluster_shift;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->cluster_loop_len = cluster_len;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->unit_loop_shift_incr = unit_shift;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->unit_loop_len = unit_len;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->inter_unit_loop_len = inter_unit_len;
    reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->dma_cmd_count = count; // (1+inter_unit_len) * (1+unit_len) * (1+cluster_len);

    total_loop_encoded_dmas += count;
    total_loops +=1 ;
    return loop_cmd;
}

bool DmaLoopExtension::verifyLoopCommand(std::list<COMMAND> &loop_cmd_list, COMMAND &loop_cmd, bool debug) {
    auto base = loop_cmd_list.front();

    if (debug) {
        printf("      [DMA LOOP Verify] In List Size: %lu \n", loop_cmd_list.size());
        printf("        Loop: %s\n", reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->to_char());
//        int cnt = 0;
...
//    //        if (cnt > 3) break;
    ...
    std::list<COMMAND> generateCmds;

    uint32_t cluster_mask = base.cluster;
    uint32_t unit_mask = base.unit_mask;
    uint32_t lm_addr = base.lm_addr;
    uint32_t mm_addr = base.mm_addr;

    int generated_cmds = 0;

    for (int cluster = 0; cluster <= reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->cluster_loop_len; ++cluster) {
        for (int unit = 0; unit <= reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->unit_loop_len; ++unit) {
            for (int inter_unit = 0; inter_unit <= reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->inter_unit_loop_len; ++inter_unit) {

                COMMAND gen(base);
                gen.mm_addr = mm_addr;
                gen.lm_addr = lm_addr;
                gen.cluster = cluster_mask;
                gen.unit_mask = unit_mask;

                generateCmds.push_back(gen);

                generated_cmds++;
                if (generated_cmds >= reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->dma_cmd_count){
                    // exit generation.
                    inter_unit = reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->inter_unit_loop_len;
                    unit = reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->unit_loop_len;
                    cluster = reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->cluster_loop_len;
                }

                lm_addr += reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->lm_incr;
                mm_addr += reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->mm_incr;
            }
            lm_addr = base.lm_addr;
            unit_mask <<= reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->unit_loop_shift_incr;
        }
        unit_mask = base.unit_mask;
        cluster_mask <<= reinterpret_cast<COMMAND_DMA_LOOP *>(&loop_cmd)->cluster_loop_shift_incr;
    }

    auto dma_compare = [](const COMMAND &a, const COMMAND &b){
        return (a.equals(b) && a.mm_addr == b.mm_addr);
    };
