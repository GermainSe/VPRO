...
void Loadbs::vproInstruction() {
    VPRO::DIM3::LOADSTORE::loadbs(getOffset(),
        getSrc().getOffset(),
        getSrc().getAlpha(),
        getSrc().getBeta(),
        getSrc().getGamma(),
...
void Loadbs::riscvInstruction(int32_t* rf,
    int16_t* lm,
    const size_t& x,
    const size_t& y,
    const size_t& z,
    ...
    // load signed!
    loaded_data = DataFormat::signed8Bit(loaded_data);  // load of 16-bit!
