...
Macl::Macl(LANE lane,
    ...
    : Processing(lane,
          ...
          update_flags,
          blocking) {}
void Macl::vproInstruction() {
    VPRO::DIM3::PROCESSING::macl(getLane(),
        getDst()->getAddress(),
        getSrc1()->getAddress(),
        getSrc2()->getAddress(),
        getXEnd(),
...
void Macl::riscvInstruction(int32_t* rf,
    int16_t* lm,
    const size_t& x,
    const size_t& y,
    const size_t& z,
...
//    printf_warning("RV MACL: 0x%06x x 0x%06x = ", src1_data, src2_data);
...
//    printf_warning("accu: %" PRIu64 " | 0x%" PRIx64 " ", accu, accu);
    ...
    accu = DataFormat::signed48Bit(accu);
    result = DataFormat::signed24Bit(accu);

//    printf_warning("%i , %i\n ", accu, result);
//    printf_warning("(RV MACL): RF[%i] = 0x%x\n", getDst()->calculateAddress(x, y, z), result);
