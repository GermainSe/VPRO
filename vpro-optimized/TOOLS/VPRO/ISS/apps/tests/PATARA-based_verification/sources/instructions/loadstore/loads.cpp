...
void Loads::vproInstruction() {
    VPRO::DIM3::LOADSTORE::loads(getOffset(),
        getSrc().getOffset(),
        getSrc().getAlpha(),
        getSrc().getBeta(),
        getSrc().getGamma(),
...
void Loads::riscvInstruction(int32_t* rf,
    int16_t* lm,
    const size_t& x,
    const size_t& y,
    const size_t& z,
    ...
    // load signed!
    loaded_data = DataFormat::signed16Bit(loaded_data);  // load of 16-bit!
