...
Mv_nz::Mv_nz(LANE lane,
    ...
    : Processing(lane,
          ...
          update_flags,
          blocking) {}
void Mv_nz::vproInstruction() {
    VPRO::DIM3::PROCESSING::mv_non_zero(getLane(),
        getDst()->getAddress(),
        getSrc1()->getAddress(),
        getSrc2()->getAddress(),
        getXEnd(),
...
void Mv_nz::riscvInstruction(int32_t* rf,
    int16_t* lm,
    const size_t& x,
    const size_t& y,
    const size_t& z,
    ...
    if (!getSrc1()->getZFlag(rf, x, y, z, ls, left, right)) {
