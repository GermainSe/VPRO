...
void Load::vproInstruction() {
    VPRO::DIM3::LOADSTORE::load(getOffset(),
        getSrc().getOffset(),
        getSrc().getAlpha(),
        getSrc().getBeta(),
        getSrc().getGamma(),
...
void Load::riscvInstruction(int32_t* rf,
    int16_t* lm,
    const size_t& x,
    const size_t& y,
    const size_t& z,
    ...
    // load unsigned!
    loaded_data = DataFormat::unsigned16Bit(loaded_data);  // load of 16-bit!
