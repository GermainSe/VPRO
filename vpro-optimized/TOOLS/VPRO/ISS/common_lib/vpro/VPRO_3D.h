        ...
        //            if(pipe.cmd->dst.imm == 0b11) pipe.lm_addr = pipe.cmd->src2.imm + pipe.cmd->src1.offset - pipe.cmd->src1.alpha * pipe.cmd->x - pipe.cmd->src1.beta * pipe.cmd->y;
            ...
            dynamic_load(uint32_t src_lane, uint32_t src_offset, uint32_t src_alpha, uint32_t src_beta, uint32_t src_gamma, uint32_t x_end, uint32_t y_end, uint32_t z_end, bool flag_update = false) {
                if (src_lane == L0) {
                    __vpro(LS, NONBLOCKING, IS_CHAIN, FUNC_LOAD, flag_update ? FLAG_UPDATE : NO_FLAG_UPDATE,
                        DST_ADDR(0, 0, 0, 0),
                        SRC_INDIRECT_LANE0(src_alpha, src_beta, src_gamma),
                        SRC2_IMM_3D(src_offset),
                        x_end, y_end, z_end);
                } else if (src_lane == L1) {
                    __vpro(LS, NONBLOCKING, IS_CHAIN, FUNC_LOAD, flag_update ? FLAG_UPDATE : NO_FLAG_UPDATE,
                        DST_ADDR(0, 0, 0, 0),
                        SRC_INDIRECT_LANE1(src_alpha, src_beta, src_gamma),
                        SRC2_IMM_3D(src_offset),
                        x_end, y_end, z_end);
                }
            }; // dynamic_load
...
//                   dst, src1, src2, x_end, y_end, z_end);
        ...
        mach_init_imm(uint32_t lane_mask, uint32_t dst, uint32_t src1, uint32_t src2, uint32_t x_end, uint32_t y_end, uint32_t z_end,
...
#ifdef SIMULATION
            if (((src2 >> ISA_COMPLEX_LENGTH_3D) & 0b11) != SRC_SEL_LS){
                printf_error("[MACH_PRE] SRC2 is not chaining from LS but Accu shall reset to imm (only possibility if chaining from LS)!\n");
            }
            ...
            __vpro(lane_mask, blocking ? BLOCKING : NONBLOCKING, is_chain ? IS_CHAIN : NO_CHAIN, FUNC_MACH,
                   update_flags ? FLAG_UPDATE : NO_FLAG_UPDATE,
                   dst, src1, SRC_IMM_3D(imm, SRC_SEL_LS), x_end, y_end, z_end);
        }
        ...
        mach_init_addr(uint32_t lane_mask, uint32_t dst, uint32_t src1, uint32_t src2, uint32_t x_end, uint32_t y_end, uint32_t z_end,
...
#ifdef SIMULATION
            if (((src2 >> ISA_COMPLEX_LENGTH_3D) & 0b11) != SRC_SEL_LS){
                printf_error("[MACH_PRE] SRC2 is not chaining from LS but Accu shall reset to addr (only possibility if chaining from LS)!\n");
            }
            ...
            __vpro(lane_mask, blocking ? BLOCKING : NONBLOCKING, is_chain ? IS_CHAIN : NO_CHAIN, FUNC_MACH,
                   update_flags ? FLAG_UPDATE : NO_FLAG_UPDATE,
                   dst, src1, complex_ADDR_3D(SRC_SEL_LS, init_offset, init_alpha, init_beta, init_gamma), x_end, y_end, z_end);
        }
        ...
        inline static void __attribute__((always_inline))
        nop(uint32_t lane_mask, uint32_t dst, uint32_t src1, uint32_t src2, uint32_t x_end, uint32_t y_end, uint32_t z_end,
            bool is_chain = false, bool update_flags = false, bool blocking = false
        ) {
            __vpro(lane_mask, blocking ? BLOCKING : NONBLOCKING, is_chain ? IS_CHAIN : NO_CHAIN, FUNC_NOP,
                update_flags ? FLAG_UPDATE : NO_FLAG_UPDATE,
                dst, src1, src2, x_end, y_end, z_end);
        }

        inline static void __attribute__((always_inline))
        nop(uint32_t lane_mask, uint32_t cnt) {
            __vpro(lane_mask, NONBLOCKING, NO_CHAIN, FUNC_NOP, NO_FLAG_UPDATE,
                DST_ADDR(0, 0, 0, 0), SRC1_IMM_3D(0), SRC2_IMM_3D(0),
                0, 0, cnt);
        }
