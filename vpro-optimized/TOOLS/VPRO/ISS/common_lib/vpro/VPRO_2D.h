...
//                       dst, src1, src2, x_end, y_end);
            ...
            mach_init_imm(uint32_t lane_mask, uint32_t dst, uint32_t src1, uint32_t src2, uint32_t x_end,
...
#ifdef SIMULATION
                if (((src2 >> ISA_COMPLEX_LENGTH_2D) & 0b11) != SRC_SEL_LS) {
                    printf_error(
                            "[MACH_PRE] SRC2 is not chaining from LS but Accu shall reset to imm (only possibility if chaining from LS)!\n");
                }
                ...
                __vpro(lane_mask, blocking ? BLOCKING : NONBLOCKING, is_chain ? IS_CHAIN : NO_CHAIN, FUNC_MACH,
                       update_flags ? FLAG_UPDATE : NO_FLAG_UPDATE,
                       dst, src1, SRC_IMM_2D(imm, SRC_SEL_LS), x_end, y_end);
            }
            ...
            mach_init_addr(uint32_t lane_mask, uint32_t dst, uint32_t src1, uint32_t src2, uint32_t x_end,
                ...
                if (((src2 >> ISA_COMPLEX_LENGTH_2D) & 0b11) != SRC_SEL_LS) {
                    printf_error(
                            "[MACH_PRE] SRC2 is not chaining from LS but Accu shall reset to addr (only possibility if chaining from LS)!\n");
                }
                ...
                __vpro(lane_mask, blocking ? BLOCKING : NONBLOCKING, is_chain ? IS_CHAIN : NO_CHAIN, FUNC_MACH,
                       update_flags ? FLAG_UPDATE : NO_FLAG_UPDATE,
                       dst, src1, complex_ADDR_2D(SRC_SEL_LS, init_offset, init_alpha, init_beta), x_end, y_end);
            }
            ...
            inline static void __attribute__((always_inline))
            nop(uint32_t lane_mask, uint32_t dst, uint32_t src1, uint32_t src2, uint32_t x_end, uint32_t y_end,
                bool is_chain = false, bool update_flags = false, bool blocking = false
            ) {
            ...
            nop(uint32_t lane_mask, uint32_t cnt) {
                __vpro(lane_mask, NONBLOCKING, NO_CHAIN, FUNC_NOP, NO_FLAG_UPDATE,
                    DST_ADDR(0, 0, 0), SRC1_IMM_2D(0), SRC2_IMM_2D(0),
                    0, cnt);
            }
