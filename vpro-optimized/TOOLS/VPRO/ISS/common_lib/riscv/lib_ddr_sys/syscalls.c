...
/*
* The following variables & addresses are used in sim_debug (HW Simulation)
*/
/* write to this reg for outputting strings */
#define STDOUT_REG 0xffffffb0
/* write test result of program to this reg */
#define RESULT_REG 0xffffffc8
/* write exit value of program to this reg */
#define EXIT_REG 0xffffffcc

/*
* The following variables & addresses are for GPR
*/
#define GPR_RUNNING 0xfffffc58
#define GPR_EXITCODE 0xfffffc5c
...
void _exit(int exit_status) {
    ...
    *(volatile int *) GPR_RUNNING = 0;
    *(volatile int *) GPR_EXITCODE = exit_status;
    asm volatile("wfi");
...
int _ftime(struct timeb *tp) {
    uint32_t hi = *((volatile uint32_t*) (0xFFFFFFF8));
    uint32_t lo = *((volatile uint32_t*) (0xFFFFFFF4));
    uint64_t cnt = ((((uint64_t)hi) << 32) + (uint64_t)lo);

    uint32_t freq = (*((volatile uint32_t*) (0xFFFFFC00 + 7*4)));
    tp->time = cnt / freq;
    tp->millitm = 1000 * cnt / freq;
    tp->timezone = 360;
    tp->dstflag = 1;
